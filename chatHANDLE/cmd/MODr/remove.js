...
if (socket.proto.rank>config.ranks.moderator.rank){
  if (selectedSocket && selectedSocket.proto.id !== socket.proto.id){
    ...
    });Tolog(socket.proto.room,'<b style="color:#00FF00">server(S)@</b>'+hours+":"+minutes+":"+seconds+" "+month+'-'+date+'-'+year+''+':'+`${selectedSocket.proto.name} is no longer a moderator`);
  } else{
    socket.emit('message',{
      name: 'server',
