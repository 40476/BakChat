if(!fs.existsSync('./users/'+fixname(message.split(' ')[1])+'.json')){
  try{
  fs.writeFileSync('./users/'+fixname(message.split(' ')[1])+'.json',`{"psk":"${hash(message.split(' ')[2],message.split(' ')[1])}","rank":${config.ranks.user.rank},"banned":false,"credit":0,"times":[],"profile":{"img":"","desc":"","bio":""}}`);
